<template>

  <div class="title_box">
    <h2>사업비 입금내역조회</h2>
    <ul class="loc">
      <li>홈</li>
      <li>집행 관리</li>
      <li>사업비 입금내역조회</li>
    </ul>
  </div>

  <div class="conbody">

    <div class="search_box top other noline">
      <div class="row">
        <div class="con d_flex">
          <span class="tit">TASK</span>
          <div class="txt tooltip_box">
            <p v-if="!taskName">선택하세요.</p> <!-- TASK 선택 전 -->
            <div v-else><!-- TASK 선택 후 활성 -->
              <p>TASK명 출력 영역 TASK명 출력 영역</p>            
              <p class="tooltip" @click="layerPopView">TASK 정보</p>
              <!-- layer pop in conbody -->
              <div class="incon_layer_pop" :class="{ 'open': layerPop }">
                <div class="layer_pop_con">
                  <div class="title">
                    <span class="ico_progress">진행</span><!-- 진행 ico_progress, 중단 ico_stop, 정산 ico_calculate, 완료 ico_comp -->
                    회원관리 UI/UX 화면 변경 진행 화면회원관리 UI/UX 화면 변경 진행 화면
                  </div>
                  <v-btn class="vbtn btn_close" @click="layerPopView">닫기</v-btn>
                  <div class="con">
                    <v-table class="vtable_view">
                      <colgroup>
                        <col style="width:80px;"/>
                        <col style="width:*;"/>
                      </colgroup>
                      <tbody>
                        <tr>
                          <th>TASK 기간</th>
                          <td>2023-10-10 ~ 2024-10-10</td>
                        </tr>
                        <tr>
                          <th>TASK 예산</th>
                          <td>10,000,000 원</td>
                        </tr>
                        <tr>
                          <th>발주기관 명</th>
                          <td>한마 생명</td>
                        </tr>
                        <tr>
                          <th>담당자</th>
                          <td>
                            <div class="d_flex">
                              <ul class="flex1">
                                <li class="tag">#발주 기관</li>
                                <li>김발주(책임자)</li>
                                <li>김신한신한(TASK 담당자)</li>
                                <li>김신한(정산 담당자)</li>
                                <li>김신한(정산 담당자)</li>
                                <li>김신한(정산 담당자)</li>
                              </ul>
                              <ul class="flex1 ml_10">
                                <li class="tag">#수행 기관</li>
                                <li>김수행(책임자)</li>
                                <li>홍길동길동(지급 담당자)</li>
                                <li>김신한(지급 담당자)</li>
                              </ul>
                            </div>
                          </td>
                        </tr>
                      </tbody>
                    </v-table>
                  </div>
                </div>
              </div>
              <!-- //layer pop -->
            </div>
          </div>
          <v-btn class="vbtn line" size="small" @click="taskOpen">선택</v-btn>
          <v-btn class="vbtn line btn_reset ml_5" size="small">초기화</v-btn>
        </div>
      </div>

      <div class="row">
        <div class="con flex1">
          <div class="input_flex a_center">
            <div class="i_calender a_center mr_20">
              <span class="tit">기간</span>
              <ComSelectBox groupCode="01" v-model="comboDate" :items="itemsDate" class="s_basics w_150 mr_10 none_details" />
              <ComDatePicker v-model="startDate" class="i_date w_145" placeholder="날짜선택" />
              <span>~</span>
              <ComDatePicker v-model="endDate" class="i_date w_145" placeholder="날짜선택" />
            </div>
            <div class="flex1">
             <div class="input_flex a_center">
              <span class="tit">가상계좌번호</span>
              <v-text-field label="가상계좌번호" v-model="vAcc" :rules="vAccRules" placeholder="가상계좌번호를 입력하세요." class="i_basics flex1 none_details"></v-text-field>                          
              <v-btn class="vbtn line search ml_5" size="small">조회</v-btn>
            </div>
            </div>
          </div>
        </div>
      </div>      
    </div>

    <div class="tit_cnt_btn">
      <p class="tit_cnt">조회 건 수 <b>100</b>건</p>
      <div class="a_center">
        <!-- <v-btn class="vbtn line" size="small">엑셀 다운로드</v-btn> -->
        <v-checkbox v-model="checkbox1" class="mr_10" :label="`취소 건`" />
        <ComSelectBox groupCode="01" v-model="comboCnt" :items="itemsCnt" class="s_basics none_details w_120" />
        <span class="unit">(단위:원)</span>
      </div>
    </div>
    <ComAgGridBasic
      ref="agrid"
      :columnDefs="columnDefsReceive"
      style="height: 570px"
      class="grid"
      :rowData="rowDataReceive"
      :defaultColDef="defaultColDefReceive"
      :rowHeight="51"
    />
    <v-btn class="vbtn btn_grid_more" size="small">더보기</v-btn>

    <!-- <div class="btn_btm_wrap">
      <v-btn class="vbtn blue" size="large">초기화</v-btn>
    </div> -->

  </div>
  
</template>

<style scoped>
  
</style>

<script setup >

import ComSelectBox from '@/components/common/ComSelectBox.vue'
import ComDatePicker from '@/components/common/ComDatePicker.vue'
import ComAgGridBasic from '~/components/common/ComAgGridBasic.vue'

// TASK 정보 레이어 팝업
const taskName = ref(true) // 화면 확인 용
const taskOpen = () => { // 화면 확인 용
  taskName.value = !taskName.value
};

const layerPop = ref(false)
const layerPopView = () => {
  layerPop.value = !layerPop.value
};

const comboDate = ref('01')
const itemsDate = [
  { title: '사업 기간', value: '01' },
  { title: '입금 기간', value: '02' },
]

const today = new Date() // 현재 날짜와 시간을 포함하는 Date 객체 생성
const year = today.getFullYear() // 년도 가져오기
const month = today.getMonth() + 1 // 월 가져오기 (0부터 시작하므로 1 더하기)
const day = today.getDate() // 일 가져오기
const formattedToday = new Date(
  `${year}-${month.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`,
)
const startDate = ref(formattedToday)
const endDate = ref(formattedToday)

const vAcc = ref('')
const vAccRules = [
  //(v) => !!v || '다시 입력해주세요.',  
]

const checkbox1 = ref(false)

const comboCnt = ref('001')
const itemsCnt = ref([
  { title: '10개씩', value: '001' },
  { title: '50개씩', value: '002' },
  { title: '100개씩', value: '003' },
])

class btnProgress {
  eGui;

  init(params) {
    this.eGui = document.createElement('div');
    this.eGui.classList.add('my-custom-cell-renderer');
    if (params.value === '진행') {
      this.eGui.innerHTML =
        `<button type="button" class="btn_vbtn btn_progress"></button>`;
    } else if (params.value === '완료') {
      this.eGui.innerHTML =
        `<button type="button" class="btn_vbtn btn_comp"></button>`;
    }    
  }

  getGui() {
    return this.eGui;
  }

  refresh(params) {
    return false;
  }
}

const columnDefsReceive = ref([
  { headerName: 'NO.', field: 'no', width: 80 },
  // { headerName: '사업명', field: 'title', width: 300, cellClass: 'ellipsis t_left' },
  { headerName: 'TASK 명', field: 'taskName', width: 260 },
  { headerName: '수행기관 명', field: 'orgName', width: 180 },
  { headerName: '사업자등록번호', field: 'corporateNum', width: 140 },
  { headerName: '책임자', field: 'director', width: 120 },
  { headerName: '계좌 구분', field: 'classAcc', width: 130 },
  { headerName: '계좌번호', field: 'accNum', width: 180 },
  { headerName: '입금 일자', field: 'depositDate', width: 130 },
  { headerName: '입금 금액', field: 'depositAmount', width: 160, cellClass: 'cell_right' },
  { headerName: '취소 일자', field: 'cancelDate', width: 100, cellRenderer: btnProgress },
])
const rowDataReceive = [
  {
    no: '999999',
    title: '차세대 공공기관 입찰 사업01 차세대 공공기관 입찰 사업',
    taskName: '회원관리회원관리회원관리회원관리',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '1,000,000,000',
    cancelDate: '진행',
  },
  {
    no: '2',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '진행',
  },
  {
    no: '3',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '완료',
  },
  {
    no: '4',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '완료',
  },
  {
    no: '5',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '',
  },
  {
    no: '6',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '진행',
  },
  {
    no: '7',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '',
  },
  {
    no: '8',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '완료',
  },
  {
    no: '9',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '',
  },
  {
    no: '10',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '',
  },
  {
    no: '11',
    title: '차세대 공공기관 입찰 사업01',
    taskName: '회원관리01',
    orgName: '한마정보통신',
    corporateNum: '123-45-68790',
    director: '김수행',
    classAcc: '정부사업비',
    accNum: '56291835234701',
    depositDate: '2023-10-10',
    depositAmount: '10,000,000',
    cancelDate: '',
  },
]
const defaultColDefReceive = {
  sortable: true,
  filter: true,
  editable: false,
  resizable: true,
  cellStyle: { textAlign: 'center' },
  // flex:1, // width auto
}
const agrid = ref(null)

</script> 